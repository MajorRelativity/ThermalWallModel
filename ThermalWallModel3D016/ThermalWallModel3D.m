...
if qAT == 1 || qAT == 2 || qAT == 3 || qAT == 5
    ...
    if qAT == 1 || qAT == 2
        ...
        parfor i = 1:ProcessNum
            ...
            thermalmodelgm = createpde('thermal',modelType); % the gm stands for the thermal model created with geometry
            
            % Thermal Geometry:
            disp(['[Process ',num2str(i),'/',num2str(ProcessNum),'] ','[$] Creating Geometry'])
            gm = modelshapew3D(thermalmodelgm,qRM,Lw,Hw,Tw,Lf,Hf,Tf);
            thermalmodelgm.Geometry = gm;
            
            ThermalModelgm{i} = thermalmodelgm % the capitals indicate that this is a cell matrix of thermal models with geometry
            
            % Completion Message:
            disp(['[Process ',num2str(i),'/',num2str(ProcessNum),'] ','[$] Geometry Completed'])
        end
        
        
        %% Assign Faces and Generate Mesh:
    
        for i = 1:ProcessNum
            
            thermalmodel = ThermalModelgm{i};
            ...
            while gateV == 0
                ...
                IndoorF = input(['[Process ',num2str(i),'/',num2str(ProcessNum),'] ','[?] Please specify the face # of the indoor side: ']);
                OutdoorFF = input(['[Process ',num2str(i),'/',num2str(ProcessNum),'] ','[?] Please specify the face # of the outdoor foam side: ']);
        ...
        if qS == 1
            save(MS,'ThermalModel','ProcessNum','Foam','-append')
        end
